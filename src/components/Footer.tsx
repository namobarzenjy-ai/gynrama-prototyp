...
const SNABBLANKAR = [
  { label: t("Boka tid", "Book an appointment", "حجز موعد"), href: "/boka-tid" },
  ...
  { label: t("Formulär & länkar", "Forms & links", "النماذج والروابط"), href: "/formular-och-lankar" },
    ...
    <footer className="mt-auto bg-slate-deep text-white">
      <div className="mx-auto max-w-[1240px] px-6 py-20">
        <div className="grid gap-12 lg:grid-cols-[1.4fr_1fr_1fr_1fr]">
          <div>
            ...
            <p className="mt-5 max-w-[38ch] text-[15px] leading-relaxed text-white/70">
              {t(
                ...
                "Everyone is welcome here, couples and single patients alike, wherever in the country or the world you live. No referral is needed and there are no waiting times.",
                "الجميع مرحّب بهم لدينا، أزواجاً وأفراداً، أينما كنتم في السويد أو حول العالم. لا حاجة إلى إحالة ولا توجد أوقات انتظار.",
              )}
            </p>

            <div className="mt-6 flex gap-2">
              <Social href={kontakt.facebook} label={t("GynRaMa på Facebook", "GynRaMa on Facebook", "GynRaMa على فيسبوك")}>
              ...
              <Social href={kontakt.instagram} label={t("GynRaMa på Instagram", "GynRaMa on Instagram", "GynRaMa على إنستغرام")}>
                <svg viewBox="0 0 24 24" fill="currentColor" className="size-4">
          ...
          <div>
            <h2 className="text-[21px]">{t("Kontakt", "Contact", "التواصل")}</h2>
            <ul className="mt-5 space-y-3 text-[15px] text-white/70">
              <li>
                <span className="block text-white/50">{t("Telefon", "Phone", "الهاتف")}</span>
                <Link href={kontakt.telefonHref} className="hover:text-lila-ljus max-lg:inline-flex max-lg:min-h-11 max-lg:items-center">
                  {kontakt.telefon}
                </Link>
              </li>
              <li>
                <span className="block text-white/50">{t("E-post", "Email", "البريد الإلكتروني")}</span>
                <Link
              ...
              <li>
                <span className="block text-white/50">{t("Adress", "Address", "العنوان")}</span>
          ...
          <div>
            <h2 className="text-[21px]">{t("Snabblänkar", "Quick links", "روابط سريعة")}</h2>
          ...
          <div>
            <h2 className="text-[21px]">{t("Öppettider", "Opening hours", "ساعات العمل")}</h2>
            <ul className="mt-5 space-y-3 text-[15px] text-white/70">
              <li>
                {kontakt.oppet}
        ...
        <div className="mx-auto flex max-w-[1240px] flex-col items-center gap-2 px-6 py-8 text-center text-[14px] text-white/55">
          <p className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1">
            <Link href={kontakt.gdpr} className="underline-offset-4 hover:text-lila-ljus hover:underline max-lg:inline-flex max-lg:min-h-11 max-lg:items-center">
              {t("Integritetspolicy (GDPR)", "Privacy policy (GDPR)", "سياسة الخصوصية (GDPR)")}
            </Link>
            ...
            <Link href={kontakt.allabolag} className="underline-offset-4 hover:text-lila-ljus hover:underline max-lg:inline-flex max-lg:min-h-11 max-lg:items-center">
              {t("Företagsinformation via Allabolag", "Company information via Allabolag", "معلومات الشركة عبر Allabolag")}
            </Link>
          </p>
          <p>
            GynRaMa AB · {t("Org.nr", "Corp. ID", "رقم الشركة")} {kontakt.orgnr} ·{" "}
            {t("Kontaktperson", "Contact person", "جهة الاتصال")}: {kontakt.verksamhetschef}
          </p>
          <p>
            {t(
              "Vid klagomål, kontakta oss via e-post",
              "For complaints, contact us by email",
              "لتقديم شكوى، تواصلي معنا عبر البريد الإلكتروني",
            )}{" "}
